...
| --- | --- | --- | --- | --- |
| CLI grammar and exit status | clap command surface; stable exit classes 0, 2-14 | public automation/human interface | `cli_automation_contract`, CLI unit tests with code/category/exit/retryability tables | help text is tested structurally, not stored as a complete golden file |
...
| User config | strict JSON schema 1 with deny-unknown fields; explicit CLI values take precedence over configured values and built-in defaults | bounded default policy for state root, provider, CPU, memory, lock/readiness/action timeouts, and human output | config unit tests and CLI error-taxonomy tests | file must be ordinary/no-follow, every existing ancestor must be reparse-free, and Unix files must be private; values outside engine and 0-30000/1-3600 timeout bounds are rejected |
