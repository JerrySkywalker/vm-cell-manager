...
$currentMarker = [string]$vm.Notes
$desiredMarker = [string]$request.ownership_marker

if ([string]$vm.Id -ne [string]$expected.id -or
    [string]$vm.Name -ne [string]$expected.name -or
    ($currentMarker -ne [string]$expected.ownership_marker -and $currentMarker -ne $desiredMarker) -or
  ...
  throw 'OWNERSHIP_CHANGED: Hyper-V VM creation receipt changed before ownership claim'
}

if ($currentMarker -ne $desiredMarker) {
  Set-VM -VM $vm -Notes $desiredMarker -AutomaticCheckpointsEnabled $false -ErrorAction Stop
}
